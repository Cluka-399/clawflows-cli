...
export async function search(query, options = {}) {
  ...
  let results = index.automations.filter(a => {
    ...
    // Normalize requires to string array
    const rawReqs = Array.isArray(a.requires) ? a.requires : [];
    const reqs = rawReqs.map(r => typeof r === 'string' ? r : (r?.capability || String(r)));
    ...
    return (
      ...
      reqs.some(r => r.toLowerCase().includes(queryLower))
    );
  });
