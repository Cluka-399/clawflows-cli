...
export async function run(name, options = {}) {
  ...
  // Scan capabilities
  const capabilities = scanCapabilities(options).capabilities;
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (step.capability) {
      ...
      console.log(`  Provider: ${provider.skill}${provider.builtIn ? ' (built-in runtime capability)' : provider.alias ? ' (installed skill alias)' : ''}`);
      
      if (options['dry-run']) {
        console.log(`  Args: ${JSON.stringify(args, null, 2)}`);
        console.log('  [DRY RUN - not executing]');
        if (step.capture) {
          context[step.capture] = { _placeholder: `Dry-run result of ${step.capability}.${step.method}`, args };
          console.log(`  → Placeholder captured as: ${step.capture}`);
        }
        ...
        const capMdPath = provider.builtIn ? null : join(provider.path, provider.skill, 'CAPABILITY.md');
        
        if (provider.builtIn) {
          console.log('');
          console.log(`  📋 Execute through the built-in Lobster/runtime ${step.capability} capability`);
          console.log('');
        } else if (existsSync(capMdPath)) {
